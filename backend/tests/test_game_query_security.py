...
def test_advanced_catalog_filters_and_sorts_stay_in_postgres() -> None:
    statement = games_router._apply_advanced_db_filters(
        select(Game),
        genre="RPG",
        developer="Studio",
        publisher="Publisher",
    ...
    statement = games_router._apply_db_sort(statement, "review_count", "desc").limit(24).offset(48)
...
def test_trailer_lookups_are_cached_and_coalesced(monkeypatch) -> None:
    ...
    async def scenario() -> None:
        games_router._trailer_cache.clear()
        monkeypatch.setattr(games_router, "find_trailer_video_id", fake_lookup)
        results = await asyncio.gather(*(
            games_router._cached_trailer_video_id("same-game", "Same Game")
            for _ in range(8)
        ))
